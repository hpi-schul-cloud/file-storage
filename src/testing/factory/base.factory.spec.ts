...
describe('BaseFactory', () => {
	...
	describe('when defining the factory', () => {
		it('should call the constructor', () => {
			...
			const factory = BaseFactory.define<typeof Constructor, UserProperties>(Constructor, () => {
				return {
		...
		it('should create an instance of the class', () => {
			const factory = BaseFactory.define<User, UserProperties>(User, () => {
				return {
					email: 'joe@example.com',
					roles: ['member'],
				};
		...
		it('should delegate transient params as a trait', () => {
			const factory = BaseFactory.define<User, UserProperties, UserTransientParams>(User, ({ transientParams }) => {
		...
		it('should delegate associations as a trait', () => {
			const factory = BaseFactory.define<User, UserProperties>(User, ({ associations }) => {
				return {
					email: 'joe@example.com',
		...
		it('should delegate associations as build option', () => {
			const factory = BaseFactory.define<User, UserProperties, UserTransientParams>(User, ({ associations }) => {
				return {
					email: 'joe@example.com',
					roles: associations.roles ?? ['member'],
				};
	...
	describe('when subclassing the factory', () => {
		class UserFactory extends BaseFactory<User, UserProperties, UserTransientParams> {
			admin() {
	...
	describe('when builing a list of objects', () => {
		it('should call the constructor for each item', () => {
			const Constructor = jest.fn();
			const factory = BaseFactory.define<typeof Constructor, UserProperties>(Constructor, ({ sequence }) => {
				return {
		...
		it('should create an instance of the class for each item', () => {
			const factory = BaseFactory.define<User, UserProperties>(User, ({ sequence }) => {
				return {
					email: `joe-${sequence}@example.com`,
					roles: ['member'],
				};
