...
export class AuthorizationClientAdapter {
	...
	public async resolveToken(token: string): Promise<AccessTokenPayloadResponse> {
		try {
			const options = this.createOptionParams();

			const response = await this.authorizationApi.authorizationReferenceControllerResolveToken(token, options);
